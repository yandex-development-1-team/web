      ...
      <NavLink to={route}>
        {({ isActive }) => (
          <div
            ...
            className={`
            rounded-[12px] flex items-center gap-[8px] cursor-pointer
            hover:bg-grey-extra-light group active:bg-system-background relative
            transition-[background-color,padding] duration-400 ease-in-out
            ${isExpanded ? 'pr-[8px] pl-[12px]' : 'pr-[10px] pl-[10px]'}
          ...
          >
            <div className="w-[60px] h-[60px] flex-shrink-0">
              <Icon
                className={`
                  w-full h-full text-text group-active:text-yellow-accent-light
                  transition-[color] duration-400 ease-in-out
                `}
              />
            </div>
            ...
            {childrenItems?.length && (
              <div
                ...
                className={`
                  ...
                  overflow-hidden transition-[width,opacity] duration-400 ease-in-out
                  ${isExpanded ? 'w-[32px] opacity-100' : 'w-0 opacity-0'}
                `}
              >
                <ArrowIcon
                  className={`
                    w-[24px] h-[24px] text-text transition-transform duration-400 ease-in-out
                    ${isOpen ? '-rotate-180' : ''}
                  `}
                />
              </div>
                  ...
                  {({ isActive }) => (
                    <div
                      className={`
                        ...
                        transition-[background-color,border-color] duration-400 ease-in-out
