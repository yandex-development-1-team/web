...
export const Boxes = ({ boxesList, onDelete, onEdit, pagination, className, ...props }: BoxesProps) => {
  return (
    <div className={cn('flex flex-col gap-10', className)} {...props}>
      <div className="grid grid-cols-[repeat(auto-fill,344px)] gap-5">
        {boxesList?.map(box => {
          return <Box box={box} key={box.id} onDelete={() => onDelete(box.id)} onEdit={() => onEdit(box.id)} />
        })}
      </div>
      {pagination}
    </div>
